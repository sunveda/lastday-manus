...
export default function Dashboard() {
  ...
  const enableScheduledSync = trpc.lastday.github.enableScheduledSync.useMutation({
    onSuccess: result => toast.success("Scheduled refresh enabled", { description: result.nextExecutionAt ? `Next batch: ${new Date(result.nextExecutionAt).toLocaleString()}` : "LastDay will refresh every 12 hours." }),
      ...
      <div className="mx-auto max-w-7xl space-y-7 pb-12">
        ...
        {overview.isLoading ? (
          ...
          <section className="grid overflow-hidden rounded-[1.75rem] border border-slate-800 bg-[#12172a] lg:grid-cols-[1.05fr_0.95fr]">
            <div className="p-6 sm:p-10"><Badge className="border-amber-300/20 bg-amber-300/10 text-amber-200">Safe by design</Badge><h2 className="mt-5 max-w-md text-2xl font-semibold tracking-tight text-white">Connect only the work you choose.</h2><p className="mt-3 max-w-md text-sm leading-6 text-slate-400">LastDay uses a read-only GitHub App. You select repositories during installation, and you can revoke access in GitHub at any time.</p><Button onClick={() => connect.mutate()} disabled={connect.isPending} className="mt-7 h-11 rounded-xl bg-white px-5 font-semibold text-slate-950 hover:bg-slate-100"><Github className="mr-2 h-4 w-4" />{connect.isPending ? "Opening GitHub…" : "Connect GitHub securely"}</Button></div>
